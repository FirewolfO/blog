...
const router = createRouter({ history: createWebHistory(), routes: [
  ...
  { path: '/', component: () => import('@/layouts/AppLayout.vue'), children: [
	...
	{ path: 'reviews', name: 'reviews', component: () => import('@/views/ReviewsView.vue'), meta: { title: '内容审核' } },
...
router.beforeEach(async (to) => { document.title = `${String(to.meta.title || '内部博客')} - 内部博客`; const auth = useAuthStore(); await auth.hydrate(); if (to.meta.public) return auth.authenticated && to.name === 'login' ? '/dashboard' : true; if (!auth.authenticated) return { name: 'login', query: { redirect: to.fullPath } }; if (to.name === 'reviews' && !auth.canReview) return '/explore'; if (to.name === 'categories' && !auth.isAdmin) return '/explore'; return true })
